...
把根目录 `vault/`（VAULT，概念归 vault 插件）中的每个文件映射为 `wiki/vault/` 的 wiki 代理页。桥接件：语义依赖两端概念插件（vault 与 wiki），depends 显式声明。代理是「资产在 md 世界的代表」：登记 + 一行描述起步，摘要是可选增强。哈希是失配探测器，不是执法器——原文变化的处置走 check 分诊。
