...
- 孤儿判定是图性质：无入链且无 related 引用的页面才算孤儿；**入链源只计概念页**——index / hot / log / tags 与 archive/ 等派生页不算链接源（否则索引链接一切，孤儿永不触发）；代理页（vault/ 下）暂无入链是登记常态，降为信息级；hot 是唯一手写链接的派生页，断链受检但不作入链源
