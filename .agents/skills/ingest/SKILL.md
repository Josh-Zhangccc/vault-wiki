...
6. **写后管道**（确定性，机械自动不询问）：`python .meta/scripts/pipeline.py index` → `tags` → `hot map "<wikilink + 一句话核心>"` → `log map "<一句话>"` → `verify`（写后自证，未过即回修）；毕即按提交纪律入库（`摄入: <资产名>`，见 `.meta/protocol/actions.md`）
7. 回报：路径 / 哈希 / 描述 / tags
